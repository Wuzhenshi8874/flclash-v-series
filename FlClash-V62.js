...
// 架构:动态地区自动/手动组 + 34 业务策略组 + 341 个有效 rule-providers
// 100%+ 服务覆盖
...
//
// === v59-xbox-microsoft-unified-compatibility-fix 修改记录(2026-08-20) ===
// 1. 修复 Xbox 同一次会话被拆到 🕹️ 国外游戏 / Ⓜ️ 微软服务 / 🛠️ 工具与服务，因出口 IP 不一致导致启动转圈、登录失败和连接失败的问题。
// 2. Xbox Android App、Xbox PC App、Gaming Services、Game Bar、Microsoft Store 进程统一前置到现有 Ⓜ️ 微软服务，不新增策略组、不改变默认选择。
// 3. 按微软官方端点补齐 Xbox Live、XSTS、动态配置、商店目录、授权、购买、推送和图片链路；Xbox 游戏厂商内容仍保留在 🕹️ 国外游戏。
// 4. Windows NCSI 与 Teredo/IPv6 探测保持 DIRECT，并使用国内 DNS，避免 TUN 下出现“实际联网但系统判定离线”或 Xbox 网络检测失效。
// 5. Xbox 实时服务补充干净境外 DNS、Fake-IP 例外和强制域名嗅探；不全局放行微软遥测，不关闭广告拦截，不修改 UDP/QUIC 与 TUN 主结构。
// 6. 保留 V58 中国域名/App 兼容、V56 ASN、迅雷、MineRadio、旧版自动测速及全部现有策略组结构。
//
// === v60-flclash-microsoft-uwp-realip-fix 修改记录(2026-08-20) ===
// 1. 修复 Windows 版 FlClash 下浏览器可用，但 Microsoft Store / Xbox / Gaming Services 仍转圈或提示无法连接的 UWP Fake-IP 兼容问题。
// 2. 将微软商店、账号登录、Windows、Office 与 Xbox 核心后缀加入 fake-ip-filter，返回服务器真实 IP，避免 UWP 对 198.18.x.x Fake-IP 兼容不良。
// 3. 真实 IP 仅改变 DNS 应答，不等于 DIRECT；现有规则继续将微软功能流量送入 Ⓜ️ 微软服务。
// 4. 为相同域名补充干净境外 DNS 策略和强制域名嗅探，保证 TUN 下使用真实 IP 时仍能稳定命中原微软分流。
// 5. 不新增策略组或规则源，不修改默认节点、自动测速、广告拦截、IPv6 开关、UDP/QUIC 或 FlClash TUN UI 逻辑。
//
// === v61-fast-failover-rule-maintenance 修改记录(2026-08-28) ===
// 1. 核心自动组缩短检测周期和单次超时，并设置 max-failed-times，减少坏节点持续占用连接。
// 2. 地区自动组与全球家宽组同步启用 HTTP 204 校验和快速失败；不修改组名、节点或默认选择。
// 3. 修复 V55 将三条精确硬拦截移动到宽泛业务规则之后、导致实际无法命中的优先级回归。
// 4. 修复 fake-ip-filter-mode:rule 下混入普通通配符语法的问题；whitelist 自动等价转换为 rule 模式以支持微软真实 IP 例外。
// 5. 自动删除未被最终规则引用的 rule-provider，减少 46 个无效资源的下载、解析和定时刷新开销。
// 6. 保留 V60 微软 UWP、V59 Xbox、V58 中国访问、迅雷、MineRadio、节点选择和业务分流主体。
//
// === v61.1-audit-safety-fix 修改记录(2026-09-06) ===
// 1. 修复海外节点因“中国电信/联通/移动”等线路标签被误判为 CN：明确国家/地区优先，运营商词仅作为无地区信息时的弱兜底。
// 2. 修复 Fake-IP rule/whitelist 转换语义漂移：保留原 MATCH 兜底动作，支持 rule-set:/geosite: 域名集合，并按 Mihomo 通配符语义转换复杂 * / +. / . 前缀。
// 3. 修复 rule-provider 清理遗漏 DNS 引用：删除前递归扫描 dns 中 RULE-SET 与 rule-set: 引用，避免 fake-ip-filter 等仍引用的 provider 被误删。
// 4. 修复 dns: {} 空对象未补基础字段：仅对缺失字段补 enable/enhanced-mode/fallback 等默认值，不覆盖用户已明确设置的值。
// 5. 不新增/删除策略组，不修改节点选择结构、测速参数、业务组默认项、广告后置、TUN/Sniffer 主逻辑。
//
// === v61.2-no-direct-fallback-safety-fix 修改记录(2026-09-06) ===
// 1. 在清理机场原 proxy-groups / rules / rule-providers 之前先检查真实有效代理节点；仅剩“剩余流量/套餐到期/官网”等提示节点时立即明确报错。
// 2. 无有效节点时不再继续重建策略组，因此不会把 🌍 国外网站、AI、流媒体等组静默退化成仅 DIRECT。
// 3. 移除 VidHub 补丁中的 [🎯 节点选择, DIRECT] 隐藏兜底；找不到活动代理组时跳过该补丁并记录错误，不强行注入 DIRECT。
// 4. 保留 V61.1 的节点地区判断、Fake-IP、DNS rule-provider 引用保护和 dns:{} 默认补齐修复；不修改测速参数与正常策略组结构。
//
// === 导入方法(FlClash,两步操作) ===
//
// 必须先「创建」脚本再「关联」到订阅!
...
// 方式 A(URL):填入 https://raw.githubusercontent.com/Wuzhenshi8874/flclash-v-series/main/
// FlClash-V61.js
// 方式 B(粘贴):打开上方 GitHub 链接,全选复制粘贴
// 方式 C(jsdelivr):https://cdn.jsdelivr.net/gh/Wuzhenshi8874/flclash-v-series@main/
// FlClash-V61.js
// 保存。
//
// 【第 2 步:关联到订阅】
...
// 版本常量
// ================================================================
const VERSION = 'v5.3.2-flclash.1-qure-iconfix.61.2-no-direct-fallback-safety-fix'
...
const CN_WEAK_CARRIER_KEYWORDS = ['电信', '联通', '移动', '铁通', 'chinatelecom', 'chinaunicom', 'chinamobile']
function _isWeakCnCarrierKeyword(keyword) {
var k = String(keyword || '').toLowerCase()
return CN_WEAK_CARRIER_KEYWORDS.indexOf(k) !== -1
}
function _isCnCarrierBrandContext(nameStr, keyword) {
var k = String(keyword || '').toLowerCase()
if (k === '中国') return /中国(?:移动|联通|电信|铁通)/.test(nameStr)
if (k === 'china') return /\bchina[\s_-]*(?:mobile|unicom|telecom|tietong)\b/i.test(nameStr)
return false
}
const _compiledRegions = REGION_DB.map(function(region) {
var matchers = []
for (var i = 0; i < region.iso.length; i++) {
matchers.push({ type: 'iso', regex: _getWordBoundaryRegex(region.iso[i], true), keyword: region.iso[i], weakCnCarrier: false })
}
for (var j = 0; j < region.kw.length; j++) {
var kw = region.kw[j]
var weakCnCarrier = region.id === 'CN' && _isWeakCnCarrierKeyword(kw)
if (_isChinese(kw)) { matchers.push({ type: 'cn', text: kw, keyword: kw, weakCnCarrier: weakCnCarrier }) }
else { matchers.push({ type: 'en', regex: _getWordBoundaryRegex(kw, false), keyword: kw, weakCnCarrier: weakCnCarrier }) }
}
...
var weakCnCarrierMatched = false
...
var matched = m.type === 'cn' ? nameStr.indexOf(m.text) !== -1 : m.regex.test(nameStr)
if (!matched) continue
// “中国移动 / China Mobile”里的“中国 / China”也是运营商品牌的一部分，不能单独当作落地中国证据。
var carrierBrandContext = region.id === 'CN' && _isCnCarrierBrandContext(nameStr, m.keyword)
if (m.weakCnCarrier || carrierBrandContext) {
weakCnCarrierMatched = true
continue
}
return region.id
}
}
// 只有完全没有明确国家/地区信息时，才允许运营商标签作为 CN 的弱兜底。
if (weakCnCarrierMatched) return 'CN'
return null
}
...
interval: 60, tolerance: 30, timeout: 2000, 'max-failed-times': 1, 'expected-status': 204,
lazy: true, icon: iconForGroupName(name), proxies: proxies.slice() }
...
if (nodes.length > 0) {
var names = makeFusionRegionGroupNames(def.label)
var autoGroup = { name: names.auto, type: 'url-test', url: 'https://www.gstatic.com/generate_204', interval: 120, tolerance: 50, timeout: 2000, 'max-failed-times': 1, 'expected-status': 204, lazy: true, icon: iconForGroupName(names.auto), proxies: nodes.slice() }
...
var homeAutoGroup = { name: hnames.homeAuto, type: 'url-test', url: 'https://www.gstatic.com/generate_204', interval: 120, tolerance: 50, timeout: 2000, 'max-failed-times': 1, 'expected-status': 204, lazy: true, icon: iconForGroupName(hnames.homeAuto), proxies: homeNodes.slice() }
...
{ name: FUSION_AUTO.AUTO, type: 'url-test', url: 'https://www.gstatic.com/generate_204', interval: 60, tolerance: 30, timeout: 2000, 'max-failed-times': 1, 'expected-status': 204, lazy: false, icon: iconForGroupName(FUSION_AUTO.AUTO), proxies: overseasAutoProxies.slice() },
{ name: FUSION_AUTO.FAST, type: 'url-test', url: 'https://www.gstatic.com/generate_204', interval: 30, tolerance: 0, timeout: 1500, 'max-failed-times': 1, 'expected-status': 204, lazy: false, icon: iconForGroupName(FUSION_AUTO.FAST), proxies: overseasAutoProxies.slice() },
{ name: FUSION_AUTO.FALLBACK, type: 'fallback', url: 'https://www.gstatic.com/generate_204', interval: 10, timeout: 1500, 'max-failed-times': 1, 'expected-status': 204, lazy: false, icon: iconForGroupName(FUSION_AUTO.FALLBACK), proxies: overseasAutoProxies.slice() },
{ name: FUSION_AUTO.BALANCE, type: 'load-balance', url: 'https://www.gstatic.com/generate_204', interval: 180, timeout: 2000, 'max-failed-times': 2, 'expected-status': 204, strategy: 'consistent-hashing', lazy: true, icon: iconForGroupName(FUSION_AUTO.BALANCE), proxies: overseasAutoProxies.slice() },
...
if (standardProxies.length === 0) {
log(`[${VERSION}] ERROR: VidHub patch found no active proxy group; DIRECT fallback is disabled, patch skipped`)
return
}
...
// ================================================================
// 模块 H4.13: Xbox / Microsoft 全链路统一兼容补丁(v59)
// 目标：Xbox App、Gaming Services、微软账号与 Microsoft Store 的关键请求使用同一策略组，
// 避免令牌、设备校验、目录/授权接口因出口 IP 分裂而循环加载或判定连接失败。
// 仅补微软官方功能端点；不全局放行遥测，不改变策略组默认项、UDP/QUIC 或 TUN 主结构。
// ================================================================
function injectXboxMicrosoftUnifiedCompatibilityFix(config) {
var obsoleteRules = [
// V58 旧规则会把 Xbox App 主体送入国外游戏，而登录域名进入微软服务，造成同会话出口分裂。
`PROCESS-NAME,com.microsoft.xboxone.smartglass,${BIZ.GAME_INTL}`,
// V46 的动态配置临时归入工具组；V59 收口到微软服务。
`DOMAIN,settings-win.data.microsoft.com,${BIZ.TOOLS}`
]
if (!Array.isArray(config.rules)) config.rules = []
for (var o = 0; o < obsoleteRules.length; o++) {
var oldRule = obsoleteRules[o]
for (var oi = config.rules.length - 1; oi >= 0; oi--) {
if (config.rules[oi] === oldRule) config.rules.splice(oi, 1)
}
}

// NCSI 负责 Windows 的“是否联网”判定；Teredo/IPv6 探测必须走真实本地网络。
var directRules = [
'DOMAIN-SUFFIX,msftconnecttest.com,DIRECT',
'DOMAIN-SUFFIX,msftncsi.com,DIRECT',
'DOMAIN-SUFFIX,ipv6.microsoft.com,DIRECT'
]

// Android Xbox、Windows Xbox/Gaming Services、Game Bar 与微软商店全部进入同一微软策略组。
var processRules = [
`PROCESS-NAME-REGEX,(?i)^com\\.microsoft\\.xbox.*$,${BIZ.MS}`,
`PROCESS-NAME-REGEX,(?i)^com\\.gamepass.*$,${BIZ.MS}`,
`PROCESS-NAME-REGEX,(?i)^(XboxPcApp|XboxAppServices|GamingServices|GamingServicesNet|XboxGameBar|GameBar|GameBarFTServer|XboxIdentityProvider|Xbox\\.TCUI|WinStore\\.App|StorePurchaseApp)(\\.exe)?$,${BIZ.MS}`
]

// Xbox 官方服务域；游戏厂商内容仍由原 Xbox provider 留在国外游戏组。
var xboxSuffixes = [
'xboxlive.com', 'xboxservices.com', 'xboxservice.com', 'xbox.com',
'xboxlive.cn', 'xboxab.com', 'gamepass.com', 'xboxgamepass.com'
]

// 微软官方文档列出的登录、Xbox 动态配置、Store 目录/授权/购买/推送关键端点。
var microsoftCoreDomains = [
'login.live.com', 'account.live.com', 'signup.live.com', 'logincdn.msauth.net',
'login.microsoftonline.com', 'login.microsoft.com', 'account.microsoft.com',
'settings.data.microsoft.com', 'settings-win.data.microsoft.com',
'storesdk.dsx.mp.microsoft.com', 'storeedgefd.dsx.mp.microsoft.com',
'paymentinstruments.mp.microsoft.com', 'store-images.s-microsoft.com',
'img-prod-cms-rt-microsoft-com.akamaized.net',
'storecatalogrevocation.storequality.microsoft.com', 'manage.devcenter.microsoft.com',
'tsfe.trafficshaping.dsp.mp.microsoft.com'
]
var microsoftCoreSuffixes = [
'msauth.net', 'msauthimages.net', 'msftauth.net', 'msftauthimages.net', 'msidentity.com',
'displaycatalog.mp.microsoft.com', 'licensing.mp.microsoft.com',
'collections.mp.microsoft.com', 'purchase.mp.microsoft.com', 'wns.windows.com'
]

var rules = directRules.concat(processRules)
for (var xs = 0; xs < xboxSuffixes.length; xs++) {
rules.push(`DOMAIN-SUFFIX,${xboxSuffixes[xs]},${BIZ.MS}`)
}
for (var md = 0; md < microsoftCoreDomains.length; md++) {
rules.push(`DOMAIN,${microsoftCoreDomains[md]},${BIZ.MS}`)
}
for (var ms = 0; ms < microsoftCoreSuffixes.length; ms++) {
rules.push(`DOMAIN-SUFFIX,${microsoftCoreSuffixes[ms]},${BIZ.MS}`)
}
_fusionPrependUniqueRules(config, rules)
...
if (!config.dns['nameserver-policy'] || typeof config.dns['nameserver-policy'] !== 'object') config.dns['nameserver-policy'] = {}
var cleanForeignDns = FUSION_FOREIGN_DNS.slice()
var localMicrosoftDns = FUSION_CN_DNS.slice()

// 中国 Xbox 域名和系统联网探测用国内 DNS；其余 Xbox/微软认证链路使用干净境外 DNS。
for (var xd = 0; xd < xboxSuffixes.length; xd++) {
var xboxSuffix = xboxSuffixes[xd]
config.dns['nameserver-policy']['+.' + xboxSuffix] = xboxSuffix === 'xboxlive.cn' ? localMicrosoftDns.slice() : cleanForeignDns.slice()
}
for (var mcd = 0; mcd < microsoftCoreDomains.length; mcd++) {
config.dns['nameserver-policy'][microsoftCoreDomains[mcd]] = cleanForeignDns.slice()
}
for (var mcs = 0; mcs < microsoftCoreSuffixes.length; mcs++) {
config.dns['nameserver-policy']['+.' + microsoftCoreSuffixes[mcs]] = cleanForeignDns.slice()
}
config.dns['nameserver-policy']['+.msftconnecttest.com'] = localMicrosoftDns.slice()
config.dns['nameserver-policy']['+.msftncsi.com'] = localMicrosoftDns.slice()
config.dns['nameserver-policy']['+.ipv6.microsoft.com'] = localMicrosoftDns.slice()
config.dns['direct-nameserver'] = _fusionMergeUniqueArray(config.dns['direct-nameserver'], localMicrosoftDns)
config.dns['direct-nameserver-follow-policy'] = true

// Xbox 实时服务、系统联网探测与 Teredo 使用真实 IP，避免 Fake-IP 干扰 UWP/网络状态判断。
var realIpDomains = [
'xboxlive.com', '*.xboxlive.com', 'xboxservices.com', '*.xboxservices.com',
'xboxservice.com', '*.xboxservice.com', 'xbox.com', '*.xbox.com',
'xboxlive.cn', '*.xboxlive.cn', 'gamepass.com', '*.gamepass.com',
'xboxgamepass.com', '*.xboxgamepass.com',
'msftconnecttest.com', '*.msftconnecttest.com', 'msftncsi.com', '*.msftncsi.com',
'ipv6.microsoft.com', '*.ipv6.microsoft.com'
]
config.dns['fake-ip-filter'] = _fusionMergeUniqueArray(config.dns['fake-ip-filter'], realIpDomains)

// 真实 IP 模式下继续通过 TLS/QUIC SNI 恢复域名，确保规则仍稳定命中微软服务组。
if (!config.sniffer || typeof config.sniffer !== 'object') config.sniffer = {}
var forceDomains = []
for (var fd = 0; fd < xboxSuffixes.length; fd++) forceDomains.push('+.' + xboxSuffixes[fd])
for (var fmd = 0; fmd < microsoftCoreDomains.length; fmd++) forceDomains.push(microsoftCoreDomains[fmd])
for (var fms = 0; fms < microsoftCoreSuffixes.length; fms++) forceDomains.push('+.' + microsoftCoreSuffixes[fms])
config.sniffer['force-domain'] = _fusionMergeUniqueArray(config.sniffer['force-domain'], forceDomains)

log(`[${VERSION}] Xbox/Microsoft unified compatibility fix injected: rules=${rules.length}, Xbox suffixes=${xboxSuffixes.length}, Microsoft endpoints=${microsoftCoreDomains.length + microsoftCoreSuffixes.length}, total=${config.rules.length}`)
}


// ================================================================
// 模块 H4.14: FlClash Microsoft UWP 真实 IP 兼容补丁(v60)
// 目标：解决 Windows 版 FlClash 下 Microsoft Store / Xbox / Gaming Services
// 对 Fake-IP(198.18.x.x) 兼容不良导致的转圈、登录失败和“无法连接”。
// 注意：真实 IP 只改变 DNS 应答，不改变 rules 的出口选择；
// 微软功能流量仍由现有规则进入 Ⓜ️ 微软服务，不会因为返回真实 IP 就变成 DIRECT。
// ================================================================
function _fakeIpWildcardToRegex(pattern) {
var s = String(pattern || '')
var out = '^'
for (var i = 0; i < s.length; i++) {
var ch = s.charAt(i)
if (ch === '*') {
var fullLabel = (i === 0 || s.charAt(i - 1) === '.') && (i === s.length - 1 || s.charAt(i + 1) === '.')
out += fullLabel ? '[^.]+': '[^.]*'
continue
}
if (/[-/\\^$+?.()|[\]{}]/.test(ch)) out += '\\' + ch
...
function _fakeIpPatternToRule(pattern, action) {
var s = String(pattern || '').trim()
if (!s) return null
// 已经是 rule 模式语法时保持原样，尤其不能改写其 fake-ip / real-ip 动作。
if (/^(RULE-SET|GEOSITE|DOMAIN|DOMAIN-SUFFIX|DOMAIN-KEYWORD|DOMAIN-REGEX|DOMAIN-WILDCARD|MATCH),/i.test(s)) return s
// blacklist/whitelist 支持的域名集合语法，转换成 rule 模式的等价写法。
var ruleSet = /^rule-set:(.+)$/i.exec(s)
if (ruleSet && ruleSet[1].trim()) return 'RULE-SET,' + ruleSet[1].trim() + ',' + action
var geosite = /^geosite:(.+)$/i.exec(s)
if (geosite && geosite[1].trim()) return 'GEOSITE,' + geosite[1].trim() + ',' + action
// +.example.com 等价 DOMAIN-SUFFIX，会同时匹配根域与任意层级子域。
if (s.indexOf('+.') === 0 && s.length > 2) return 'DOMAIN-SUFFIX,' + s.slice(2) + ',' + action
// .example.com 只匹配子域、不匹配根域；用正则保持 Mihomo 通配符原意。
if (s.charAt(0) === '.' && s.length > 1) {
var suffix = s.slice(1).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
return 'DOMAIN-REGEX,^.+\\.' + suffix + '$,' + action
}
// * 在普通 fake-ip-filter 语义里只跨一个域名层级；复杂通配符统一转换为等价正则。
if (s.indexOf('*') !== -1) return 'DOMAIN-REGEX,' + _fakeIpWildcardToRegex(s) + ',' + action
return 'DOMAIN,' + s + ',' + action
}
function _getFakeIpRuleFallback(items, defaultAction) {
if (!Array.isArray(items)) return defaultAction
for (var i = items.length - 1; i >= 0; i--) {
var m = /^MATCH\s*,\s*(fake-ip|real-ip)\s*$/i.exec(String(items[i] || '').trim())
if (m) return m[1].toLowerCase()
}
return defaultAction
}

function injectFlClashMicrosoftUwpRealIpCompatibilityFix(config, originalFakeIpMode, originalFakeIpFilter) {
if (!Array.isArray(originalFakeIpFilter)) originalFakeIpFilter = []
if (!config.dns || typeof config.dns !== 'object') config.dns = {}
if (!Array.isArray(config.dns['fake-ip-filter'])) config.dns['fake-ip-filter'] = []
if (!config.dns['nameserver-policy'] || typeof config.dns['nameserver-policy'] !== 'object') config.dns['nameserver-policy'] = {}

// V 系列默认使用 blacklist；同时兼容 Mihomo 的 whitelist / rule 模式，不强行改写用户原模式。
if (typeof config.dns['fake-ip-filter-mode'] === 'undefined') config.dns['fake-ip-filter-mode'] = 'blacklist'
var fakeIpFilterMode = String(originalFakeIpMode || config.dns['fake-ip-filter-mode'] || 'blacklist').toLowerCase()

// 只覆盖微软功能链路，不将 Azure 公共 CDN 等共享域全量放入例外。
var microsoftUwpRealIpSuffixes = [
'microsoft.com', 'microsoftonline.com', 'microsoftonline-p.com',
'live.com', 'msauth.net', 'msauthimages.net', 'msftauth.net', 'msftauthimages.net',
'msidentity.com', 'msftidentity.com',
'windows.com', 'windowsupdate.com',
'office.com', 'office.net',
'microsoftapp.net', 's-microsoft.com',
'xboxlive.com', 'xboxservices.com', 'xboxservice.com', 'xbox.com', 'xboxab.com',
'gamepass.com', 'xboxgamepass.com'
]

var realIpPatterns = []
var forceDomains = []
var cleanForeignDns = FUSION_FOREIGN_DNS.slice()
for (var i = 0; i < microsoftUwpRealIpSuffixes.length; i++) {
var suffix = microsoftUwpRealIpSuffixes[i]
realIpPatterns.push(suffix)
realIpPatterns.push('*.' + suffix)
forceDomains.push('+.' + suffix)
config.dns['nameserver-policy']['+.' + suffix] = cleanForeignDns.slice()
}

// 微软商店图片链路使用 Akamai 独立域名，只精准例外官方主机，不放宽整个 akamaized.net。
var microsoftUwpExactDomains = [
'img-prod-cms-rt-microsoft-com.akamaized.net'
]
for (var e = 0; e < microsoftUwpExactDomains.length; e++) {
var exactDomain = microsoftUwpExactDomains[e]
realIpPatterns.push(exactDomain)
forceDomains.push(exactDomain)
config.dns['nameserver-policy'][exactDomain] = cleanForeignDns.slice()
}

if (fakeIpFilterMode === 'rule' || fakeIpFilterMode === 'whitelist') {
// rule 模式只能使用规则语法。whitelist 转为等价 rule 模式后，微软真实 IP 规则
// 可以优先于用户原有的宽白名单（例如 *.com）生效；重复执行脚本时保持上次 MATCH 兜底不变。
var realIpRules = []
for (var rr = 0; rr < microsoftUwpRealIpSuffixes.length; rr++) {
realIpRules.push('DOMAIN-SUFFIX,' + microsoftUwpRealIpSuffixes[rr] + ',real-ip')
}
for (var re = 0; re < microsoftUwpExactDomains.length; re++) {
realIpRules.push('DOMAIN,' + microsoftUwpExactDomains[re] + ',real-ip')
}
var originalLookup = {}
for (var ol = 0; ol < originalFakeIpFilter.length; ol++) originalLookup[String(originalFakeIpFilter[ol] || '').trim()] = true
var fallbackAction = fakeIpFilterMode === 'whitelist' ? 'real-ip' : _getFakeIpRuleFallback(originalFakeIpFilter, 'fake-ip')
if (fakeIpFilterMode === 'rule') fallbackAction = _getFakeIpRuleFallback(config.dns['fake-ip-filter'], fallbackAction)
var normalizedRules = realIpRules.slice()
for (var nr = 0; nr < config.dns['fake-ip-filter'].length; nr++) {
var oldItem = String(config.dns['fake-ip-filter'][nr] || '').trim()
if (!oldItem || realIpRules.indexOf(oldItem) !== -1 || /^MATCH\s*,/i.test(oldItem)) continue
var action = fakeIpFilterMode === 'whitelist' && originalLookup[oldItem] ? 'fake-ip' : 'real-ip'
var normalized = _fakeIpPatternToRule(oldItem, action)
if (normalized && normalizedRules.indexOf(normalized) === -1) normalizedRules.push(normalized)
}
normalizedRules.push('MATCH,' + fallbackAction)
config.dns['fake-ip-filter-mode'] = 'rule'
config.dns['fake-ip-filter'] = normalizedRules
fakeIpFilterMode = 'rule'
} else {
// 默认 blacklist 保持原模式，只追加微软真实 IP 域名；不触发 rule 转换。
config.dns['fake-ip-filter'] = _fusionMergeUniqueArray(config.dns['fake-ip-filter'], realIpPatterns)
}

// DIRECT 出口需要重新解析时继续服从 nameserver-policy，避免退回异常系统 DNS。
config.dns['direct-nameserver'] = _fusionMergeUniqueArray(config.dns['direct-nameserver'], FUSION_CN_DNS)
config.dns['direct-nameserver-follow-policy'] = true

// TUN 收到真实 IP 连接时通过 TLS/HTTP/QUIC 恢复域名，确保仍命中原微软规则。
if (!config.sniffer || typeof config.sniffer !== 'object') config.sniffer = {}
config.sniffer['force-domain'] = _fusionMergeUniqueArray(config.sniffer['force-domain'], forceDomains)

log(`[${VERSION}] FlClash Microsoft UWP real-IP compatibility fix injected: mode=${fakeIpFilterMode}, suffixes=${microsoftUwpRealIpSuffixes.length}, exact=${microsoftUwpExactDomains.length}, real-ip patterns=${realIpPatterns.length}`)
}
...
function isV55PriorityBlockRule(rule) {
var s = String(rule || '')
return s === 'DOMAIN-REGEX,^(log|mon)[0-9A-Za-z.-]*\\.tiktokv\\.com$,REJECT' ||
s === 'DOMAIN,incoming.telemetry.mozilla.org,REJECT-DROP' ||
s === 'PROCESS-NAME,com.xiaomi.gamecenter.sdk.service,REJECT-DROP'
}

function finalizeV55BlockRulesLast(config) {
if (!Array.isArray(config.rules)) config.rules = []
var priorityBlockRules = []
var normalRules = []
var blockRules = []
var finalMatch = null
...
if (isV55PriorityBlockRule(rule)) priorityBlockRules.push(rule)
else if (isV55FinalBlockRule(rule)) blockRules.push(rule)
else normalRules.push(rule)
}

if (finalMatch === null) finalMatch = `MATCH,${BIZ.FINAL}`
config.rules.splice(0, config.rules.length)
for (var p = 0; p < priorityBlockRules.length; p++) config.rules.push(priorityBlockRules[p])
...
log(`[${VERSION}] V55 final block tier: priority=${priorityBlockRules.length}, normal=${normalRules.length}, block=${blockRules.length}, MATCH=1`)
}

function _collectDnsRuleProviderRefs(value, used) {
if (typeof value === 'string') {
var s = value.trim()
var ruleRe = /RULE-SET\s*,\s*([^,\s]+)(?:\s*,|$)/ig
var m
while ((m = ruleRe.exec(s)) !== null) { if (m[1]) used.add(m[1]) }
var setRe = /rule-set:([A-Za-z0-9._-]+)/ig
while ((m = setRe.exec(s)) !== null) { if (m[1]) used.add(m[1]) }
return
}
if (Array.isArray(value)) {
for (var i = 0; i < value.length; i++) _collectDnsRuleProviderRefs(value[i], used)
return
}
if (!value || typeof value !== 'object') return
var keys = Object.keys(value)
for (var k = 0; k < keys.length; k++) {
_collectDnsRuleProviderRefs(keys[k], used)
_collectDnsRuleProviderRefs(value[keys[k]], used)
}
}
function pruneUnusedRuleProviders(config) {
if (!config['rule-providers'] || typeof config['rule-providers'] !== 'object') return
var used = new Set()
for (var i = 0; i < config.rules.length; i++) {
var rule = String(config.rules[i] || '')
if (rule.indexOf('RULE-SET,') !== 0) continue
var parts = rule.split(',')
if (parts[1]) used.add(parts[1])
}
// DNS 也可以通过 fake-ip-filter(rule 模式)、rule-set: 域名集合、nameserver-policy 等引用 provider。
// 清理前递归扫描整个 dns，避免路由 rules 未引用但 DNS 仍在使用的 provider 被误删。
_collectDnsRuleProviderRefs(config.dns, used)
var providerKeys = Object.keys(config['rule-providers'])
var removed = 0
for (var k = 0; k < providerKeys.length; k++) {
var provider = providerKeys[k]
if (used.has(provider)) continue
delete config['rule-providers'][provider]
removed++
}
log(`[${VERSION}] Pruned ${removed} unused rule-providers, active=${Object.keys(config['rule-providers']).length}`)
}
...
// DNS：无论 dns 缺失还是 dns: {}，都按“只补缺失字段”初始化；用户明确设置的值保持不变。
if (!config.dns || typeof config.dns !== 'object' || Array.isArray(config.dns)) config.dns = {}
if (typeof config.dns.enable === 'undefined') config.dns.enable = true
if (typeof config.dns.ipv6 === 'undefined') config.dns.ipv6 = true
if (typeof config.dns['prefer-h3'] === 'undefined') config.dns['prefer-h3'] = false
if (typeof config.dns['enhanced-mode'] === 'undefined') config.dns['enhanced-mode'] = 'fake-ip'
if (typeof config.dns['fake-ip-range'] === 'undefined') config.dns['fake-ip-range'] = '198.18.0.1/16'
if (typeof config.dns['respect-rules'] === 'undefined') config.dns['respect-rules'] = true
if (typeof config.dns.nameserver === 'undefined') config.dns.nameserver = FUSION_CN_DNS.slice()
if (typeof config.dns.fallback === 'undefined') config.dns.fallback = FUSION_FOREIGN_DNS.slice()
if (typeof config.dns['proxy-server-nameserver'] === 'undefined') config.dns['proxy-server-nameserver'] = FUSION_CN_DNS.slice()
if (typeof config.dns['fallback-filter'] === 'undefined') config.dns['fallback-filter'] = { geoip: true, 'geoip-code': 'CN', ipcidr: ['240.0.0.0/4'], domain: [] }
if (typeof config.dns['fake-ip-filter'] === 'undefined') config.dns['fake-ip-filter'] = []
if (typeof config.dns['nameserver-policy'] === 'undefined') config.dns['nameserver-policy'] = {}
// v51: 追加关键 DNS 依赖；上面的默认值只在字段缺失时写入，不覆盖用户显式值。
config.dns['proxy-server-nameserver'] = _fusionMergeUniqueArray(config.dns['proxy-server-nameserver'], FUSION_CN_DNS)
if (!config.dns['fallback-filter'] || typeof config.dns['fallback-filter'] !== 'object') config.dns['fallback-filter'] = { geoip: true, 'geoip-code': 'CN' }
config.dns['fallback-filter'].domain = _fusionMergeUniqueArray(config.dns['fallback-filter'].domain, [
...
// v61.2 P0: destructive cleanup 前做有效节点预检。
// 机场有时会把“剩余流量/套餐到期/官网”等提示项伪装成 proxy；这些不能算可用代理节点。
// 若此时继续 cleanupSubscription，会先清空原策略组，再让业务组只剩 DIRECT，形成危险的静默直连。
var preflightClassified = classifyAllNodes(config.proxies)
if (!preflightClassified.ALL || preflightClassified.ALL.length === 0) {
var noProxyError = new Error(`[${VERSION}] 未发现有效代理节点：订阅可能只剩流量/到期/官网等提示节点。已停止处理并保留原配置，禁止静默降级为 DIRECT。`)
log(noProxyError.message)
throw noProxyError
}

try {
log(`[${VERSION}] Start processing, ${config.proxies.length} proxies (${preflightClassified.ALL.length} valid)`)
if (!Array.isArray(config['proxy-groups'])) config['proxy-groups'] = []
if (!Array.isArray(config.rules)) config.rules = []
var originalFakeIpMode = config.dns && config.dns['fake-ip-filter-mode'] ? String(config.dns['fake-ip-filter-mode']) : 'blacklist'
var originalFakeIpFilter = config.dns && Array.isArray(config.dns['fake-ip-filter']) ? config.dns['fake-ip-filter'].slice() : []
overwriteGeneral(config)
cleanupSubscription(config)
injectSmartFingerprint(config)
var c = preflightClassified
...
// Xbox / Microsoft 最终功能收口：保证 App、账号、商店和 Gaming Services 使用同一微软策略组。
injectXboxMicrosoftUnifiedCompatibilityFix(config)
// FlClash / Windows UWP 收口：返回微软真实 IP，并依靠嗅探保持原有分流出口。
injectFlClashMicrosoftUwpRealIpCompatibilityFix(config, originalFakeIpMode, originalFakeIpFilter)
// 所有业务补丁完成后再收口，确保广告、追踪、钓鱼、恶意域名与硬拒绝规则统一位于 MATCH 前的最后一层。
finalizeV55BlockRulesLast(config)
pruneUnusedRuleProviders(config)
