...
// 版本常量
// ================================================================
const VERSION = 'v5.3.2-flclash.1-qure-iconfix.61-fast-failover-rule-maintenance'
...
if (!s) return null
if (/^(RULE-SET|GEOSITE|DOMAIN|DOMAIN-SUFFIX|DOMAIN-KEYWORD|DOMAIN-REGEX|MATCH),/.test(s)) return s
if (s.indexOf('*.') === 0 || s.indexOf('+.') === 0) return 'DOMAIN-SUFFIX,' + s.slice(2) + ',' + action
if (s.indexOf('*') === -1) return 'DOMAIN,' + s + ',' + action
var escaped = s.replace(/[.+?^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*')
return 'DOMAIN-REGEX,^' + escaped + '$,' + action
}
...
var originalLookup = {}
for (var ol = 0; ol < originalFakeIpFilter.length; ol++) originalLookup[String(originalFakeIpFilter[ol] || '')] = true
var normalizedRules = realIpRules.slice()
for (var nr = 0; nr < config.dns['fake-ip-filter'].length; nr++) {
var oldItem = String(config.dns['fake-ip-filter'][nr] || '')
if (realIpRules.indexOf(oldItem) !== -1 || oldItem.indexOf('MATCH,') === 0) continue
...
normalizedRules.push(fakeIpFilterMode === 'whitelist' ? 'MATCH,real-ip' : 'MATCH,fake-ip')
...
var policy = config.dns && config.dns['nameserver-policy']
if (policy && typeof policy === 'object') {
var policyKeys = Object.keys(policy)
for (var p = 0; p < policyKeys.length; p++) {
var key = policyKeys[p]
if (key.indexOf('rule-set:') === 0) used.add(key.slice('rule-set:'.length))
}
}
